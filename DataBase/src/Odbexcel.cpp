...
QString Odbexcel::error;

Odbexcel::Odbexcel(QObject *parent)
	: QObject(parent)
{
}
...
bool Odbexcel::save(QString filePath, QStringList headers, QList<QStringList> data, QString comment /*= ""*/)
{
    ...
    query.prepare( sql);
    if( !query.exec()) {
		Odbexcel::printError( query.lastError());
        db.close();
        return false;
    }
...
bool Odbexcel::saveFromTable(QString filePath, QTableView *tableView, QString comment /*= ""*/)
{
  ...
  return Odbexcel::save(filePath,headers,data,comment);
}

void Odbexcel::printError(QSqlError error)
{
...
bool Odbexcel::insert(QSqlQuery& query, QString sheetName, QStringList slist)
{
