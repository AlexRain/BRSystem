...
void UiLogin::initUser()
{
	...
	ui.label_qq->setText(QString::fromLocal8Bit("QQ群：102153497"));
...
int UiLogin::fadeIn()
{
	...
	this->raise();
	{
		...
		if (result.needUpdate&&result.force_update) {
			code = DialogMsg::question(nullptr, QObject::tr("question"), QObject::tr("find a new version, please up to date."), QMessageBox::Ok);
			if (code == QMessageBox::Ok) {
				QString fileName = QApplication::applicationDirPath() + "/"+ result.nextVersion.replace(".","_")+"_install.exe";
				this->AutoUpgrade((result.download_url.toUtf8()), fileName);
			}
			return 0;
		}
		else {
			if (result.needUpdate) {
				code = DialogMsg::question(nullptr, QObject::tr("question"), QObject::tr("find a new version, please up to date."), QMessageBox::Ok | QMessageBox::Cancel);
			}
		}

		//qDebug() << "result.force_update"<< result.force_update;
...
void UiLogin::AutoUpgrade(QString url,QString fileName)
{
	QFileInfo fileInfo(fileName);
	if (fileInfo.isFile()) {
		if (!QProcess::startDetached(fileName, QStringList()))//启动主程序，主程序在其上一级目录
		{
			DialogMsg::question(this, tr("tips"), QString::fromLocal8Bit("更新失败，请重试"), QMessageBox::Ok);
		}
		qApp->exit(0);
		return;
	}

	UiProgressBar progressBar(this);
	progressBar.setUiTitle(QString::fromLocal8Bit("下载更新"));
	progressBar.setUrl(url);
	progressBar.download(fileName);
	if (QDialog::Accepted == progressBar.exec()) {
		QProcess* process = new QProcess();
		if (!process->startDetached(fileName, QStringList()))//启动主程序，主程序在其上一级目录
		{
			DialogMsg::question(this, tr("tips"), QString::fromLocal8Bit("更新失败，请重试"), QMessageBox::Ok);
		}
		qApp->exit(0);
...
void UiLogin::ShowProgressBar()
{
	QDir dir("local-chromium");
	if (dir.exists())
	{
		return;
	}
	else {
		QString fileName = QApplication::applicationDirPath() + "/local-chromium.zip";
		QFileInfo fileInfo(fileName);
		if (fileInfo.isFile()) {
			UiProgressBar::UnDecodezip(fileName);
			return;
		}
		else {
			UiProgressBar progressBar(this);
			progressBar.setUrl("http://bill.ah-fenghe.com/h5/local-chromium.zip");
			progressBar.download(fileName);
			if (QDialog::Accepted == progressBar.exec()) {
				progressBar.UnDecodezip(fileName);
				return;
			}
		}
